...
/**
 * 说明：系统设置、初始信息等 作者：f-sci 授权：bsic
 */
@Controller
@RequestMapping("/head")
public class HeadController extends BaseController {
	...
	public Object getList() throws Exception {
		...
		if (null != user) {
			...
			// 图片转换为Base64编码传到前端，实现Linux下跨域显示头像
			if (null != pdPhoto) {
				String userPhotoPath = PathUtil.getProjectpath() + pdPhoto.getString("PHOTO2");
				String Base64String = ImageAnd64Binary.getImageStr(userPhotoPath);
			...
			if (null == session.getAttribute(Const.SHOWCOUNT)) {
				session.setAttribute(Const.SHOWCOUNT,
						IniFileUtil.readCfgValue(infFilePath, "SysSet1", Const.SHOWCOUNT, "10"));// 初始系统带分页的列表每页显示条数
			}
			...
			String sysName = IniFileUtil.readCfgValue(infFilePath, "SysSet1", Const.SYSNAME, "FSCI"); // 系统名称
	...
	public Object getDataToOnline() throws Exception {
		...
		String infFilePath = PathUtil.getClasspath() + Const.SYSSET; // 配置文件路径
		String onlineIp = IniFileUtil.readCfgValue(infFilePath, "SysSet1", "onlineIp", "192.168.10.150"); // 在线管理IP
		String onlinePort = IniFileUtil.readCfgValue(infFilePath, "SysSet1", "onlinePort", "8869"); // 在线管理端口
		map.put("fhsmsSound", IniFileUtil.readCfgValue(infFilePath, "SysSet1", "fhsmsSound", "1")); // 信息提示音
		map.put("onlineAdress", onlineIp + ":" + onlinePort); // 在线管理websocket地址
		map.put("result", errInfo);
	...
	public Object hasButton() throws Exception {
		...
		Session session = Jurisdiction.getSession();
		Collection<String> shiroSet = (Collection<String>) session
				.getAttribute(Jurisdiction.getUsername() + Const.SHIROSET);
		for (String key : keys) {
	...
	public Object sysSet() throws Exception {
		...
		pd.put("sysName", IniFileUtil.readCfgValue(infFilePath, "SysSet1", Const.SYSNAME, "FSCI")); // 系统名称
