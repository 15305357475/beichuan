...
public class ShiroConfiguration {
	...
	private void loadShiroFilterChain(ShiroFilterFactoryBean factoryBean) {
		...
		*/
		...
		filterChainMap.put("/wx/**", "anon");// 微信有关的接口
		filterChainMap.put("/report/detail", "anon");// 隐患举报详情
		filterChainMap.put("/report/getInfoByID", "anon");// 注册根据身份证获取详情
		filterChainMap.put("/admin/register", "anon");
		filterChainMap.put("/admin/getAllCompamy", "anon");// 获取全部公司
		filterChainMap.put("/admin/getAllDept", "anon");// 获取本工分组下的所有部门
		filterChainMap.put("/admin/checkinfo", "anon");// 找回密码信息检查
		filterChainMap.put("/admin/retrieve", "anon");// 找回密码
		filterChainMap.put("/admin/getuserinfo", "anon");// 获取用户基本信息，用于注册
		filterChainMap.put("/appversionctrl/getLatest", "anon");// app更新检查
	...
	public DefaultWebSessionManager sessionManager() {
		...
		sessionManager.setGlobalSessionTimeout(129600000L);// 单位ms,36个小时
	 ...
	 * 以及EhCacheManager类。
	 * 2.HashedCredentialsMatcher，这个类是为了对密码进行编码的，防止密码在数据库里明码保存，当然在登录认证的生活，
	 ...
	 * 5.SecurityManager，权限管理，这个类组合了登录，登出，权限，session的处理，是个比较重要的类。
