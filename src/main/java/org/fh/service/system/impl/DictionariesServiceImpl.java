...
/**
 * 说明：按钮权限服务接口实现类 作者：f-sci 授权：bsic
 */
@Service
@Transactional // 开启事物
public class DictionariesServiceImpl implements DictionariesService {
	...
	/**
	 * 获取所有数据并填充每条数据的子级列表(递归处理)，区域定义专用
	 * 
	 * @param MENU_ID
	 * @return
	 * @throws Exception
	 */
	public List<Dictionaries> listAllDictForArea(String parentId) throws Exception {
		List<Dictionaries> dictList = this.listSubDictByParentId(parentId);
		for (Dictionaries dict : dictList) {
			dict.setTreeurl("areadefinition_list.html?DICTIONARIES_ID=" + dict.getDICTIONARIES_ID());
			dict.setSubDict(this.listAllDictForArea(dict.getDICTIONARIES_ID()));
			dict.setTarget("treeFrame");
		}
		return dictList;
	}
	...
	/**
	 * 获取所有数据并填充每条数据的子级列表(递归处理)，隐患排查基础参数定义专用
	 * 
	 * @param MENU_ID
	 * @return
	 * @throws Exception
	 */
	public List<Dictionaries> listAllDictForIns(String parentId) throws Exception {
		List<Dictionaries> dictList = this.listSubDictByParentId(parentId);
		for (Dictionaries dict : dictList) {
			dict.setTreeurl("ins_config_list.html?DICTIONARIES_ID=" + dict.getDICTIONARIES_ID());
			dict.setSubDict(this.listAllDictForIns(dict.getDICTIONARIES_ID()));
			dict.setTarget("treeFrame");
		}
		return dictList;
	}
	 ...
	 * @throws Exception
	 */
	public List<Dictionaries> getPointByBIANMA(String parentId) throws Exception {
		return dictionariesMapper.getPointByBIANMA(parentId);
	}
	...
	/**
	 * 获取所有数据并填充每条数据的子级列表(递归处理)，设备巡检专用
	 * 
	 * @param MENU_ID
	 * @return
	 * @throws Exception
	 */
	public List<Dictionaries> listAllDictForSbxj(String parentId) throws Exception {
		List<Dictionaries> dictList = this.listSubDictByParentId(parentId);
		for (Dictionaries dict : dictList) {
			dict.setTreeurl("sbxj_config_list.html?DICTIONARIES_ID=" + dict.getDICTIONARIES_ID());
			dict.setSubDict(this.listAllDictForSbxj(dict.getDICTIONARIES_ID()));
			dict.setTarget("treeFrame");
		}
		return dictList;
	}
